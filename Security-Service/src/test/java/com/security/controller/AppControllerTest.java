...
public class AppControllerTest {
	...
	@Autowired
	SecurityMockMvcRequestPostProcessors processer;
	...
	private ResultActions processApi(String getUrl2, HttpMethod methodType, String user, Employee emp,
		...
		case GET:
			try {
				response = response = mockMvs.perform(get(getUrl2).with(user(userName2).password(userPwd2).roles("USER"))); 
						//mockMvs.perform(get(getUrl2).header(HttpHeaders.AUTHORIZATION, secret));
